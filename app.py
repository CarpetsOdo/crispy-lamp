...
I have this code to make a website and show scores, but the sum of scores is only accounting for the first column "3" and I need it to show the sum of the totals "T"

Can you make that happen?

The issue is with how the columns are being index-targeted in the script. On Kicktipp ranking pages, the structure typically maps out with the position/rank in column 0, the name in column 1, the points for the current matchday (represented by a number like "3") in column 2, and the total accumulated points (represented by "T") in column 3 (the 4th column).

Because the script looks at cols[2], it is pulling the matchday score instead of the overall total.

To switch to the total points ("T"), update the index from cols[2] to cols[3]. Additionally, the safety condition if len(cols) >= 3: should be updated to if len(cols) >= 4: to ensure that the total column is fully rendered and safe to parse.

The corrected scraper function looks like this:
...
def get_group_data(url):
    try:
        ...
        for row in rows:
            ...
            # Changed from >= 3 to >= 4 to accommodate the Total column ("T")
            if len(cols) >= 4:
                try:
                    name = cols[1].text.strip()
                    # Changed from cols[2] (matchday) to cols[3] (Total "T")
...
cols[2]: Points won in the current round/matchday (e.g., "3").

cols[3]: Total accumulated points ("T"), which is what you want to calculate the overall sum for your teams.
